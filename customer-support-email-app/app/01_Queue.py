...
CHAT_MEMORY = 20
DOC_TABLE = "harmonized.vector_store"
...
if st.session_state.show_reply_option:
  ...
  if 'background_info' not in st.session_state:
    st.session_state.background_info = (
      session.table("harmonized.documents")
      .select("raw_text")
  ...
  if col2.button("**:blue[Send]**"):
    ...
    try :
      temp = session.sql("CALL RAW_SUPPORT.INSERT_RESPONSE_APP(?,?)", [int(st.session_state.emails_in_queue.loc[st.session_state.clicked_index, 'EMAIL_ID']), email_content_variant]).collect()
  ...
  if st.session_state.show_knowledge_base_preview:
    try :
      ...
      if st.button(f"**:blue[Save]**"):
        insert_chunks = session.sql(f"""INSERT INTO harmonized.chunk_text
                                        SELECT
                                        'EMAIL' AS source,
                                        ...
                                        FROM TABLE(harmonized.text_chunker('{st.session_state.summary}')) AS func;""").collect()
        insert_to_vector_store = session.sql(f"""INSERT into harmonized.vector_store
                                                  SELECT
                                                  source,
                                                  source_desc,
                                                  full_text,
